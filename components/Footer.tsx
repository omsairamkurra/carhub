...
const Footer = () => {
    return (
        <footer className="flex flex-col text-black-100 mt-5 border-t border-grey-100">
                    ...
                    {footerLinks.map((link) => (
                        <div key={link.title} className="footer__link">
                            ...
                            {link.links.map((item) => (
                                <Link key={item.title}
                                    href={item.url}
                                    className="text-grey-500">{item.title}</Link>
                            ))}
                        </div>
                    ))}
                </div>
            ...
            <div className="flex justify-between items-center flex-wrap mt-10 border-t border-grey-100 sm:px-16 py-10">
                <p className="text-base text-grey-700">
                    @{new Date().getFullYear()} CarHub.<br />
                    All Rights Reserved
                </p>
